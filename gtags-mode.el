...
(defun gtags-mode--get-root (&optional dir)
  "Determine the top dbpath of DIR.
Includes the remote prefix concatenation when applicable."
  (when-let* (((and gtags-mode--global gtags-mode--gtags))
...
When PREFIX is nil or empty, return the entire list of completions."
  (gtags-mode--exec-sync
   "--directory" (file-local-name (gtags-mode--get-root default-directory))
   "--ignore-case" "--through" "--completion" (or prefix "")))
...
name, code, file, line."
  (if-let* ((root (gtags-mode--get-root default-directory)))
      (delete nil (mapcar
		   (lambda (line)
		     (when (string-match
			    (concat
...
(defun gtags-mode-create (root-dir)
  ...
  (when-let* ((default-directory root-dir))
    (gtags-mode--exec-async 'gtags-mode--gtags)))

(defun gtags-mode-update ()
  "Update GLOBAL project database."
  (interactive)
  (when (gtags-mode--get-root default-directory)
...
(defun gtags-mode--after-save-hook ()
  ...
  (when (and buffer-file-name
	     (gtags-mode--get-root default-directory))
...
(cl-defmethod xref-backend-identifier-completion-table ((_backend (head gtags-mode)))
  "List all symbols."
  (gtags-mode--list-completions nil))

(cl-defmethod xref-backend-definitions ((_backend (head gtags-mode)) symbol)
  "List all definitions for SYMBOL."
  (gtags-mode--xref-find-symbol '("--definition") symbol))

(cl-defmethod xref-backend-references ((_backend (head gtags-mode)) symbol)
  "List all referenced for SYMBOL."
  (gtags-mode--xref-find-symbol '("--reference" "--symbol") symbol))

(cl-defmethod xref-backend-apropos ((_backend (head gtags-mode)) symbol)
...
(defun gtags-mode--imenu-advice ()
  ...
  (when (and buffer-file-name (gtags-mode--get-root default-directory))
    (gtags-mode--filter-find-symbol
...
(defun gtags-mode-eldoc (callback &rest _ignored)
  ...
  (when-let* ((inproject (gtags-mode--get-root default-directory))
...
;; project integration ===============================================
(cl-defmethod project-root ((project (head gtags-mode)))
  "Root for PROJECT."
  (cdr project))

(cl-defmethod project-files ((project (head gtags-mode)) &optional dirs)
...
(defun gtags-mode-completion-function ()
  ...
  (when-let* ((root (gtags-mode--get-root default-directory))
	      (bounds (bounds-of-thing-at-point 'symbol)))
  ...
  (cond
   (gtags-mode
    (gtags-mode--with-feature 'project
      (add-hook 'project-find-functions #'gtags-mode-try-gtags))
    (gtags-mode--with-feature 'xref
      (add-hook 'xref-backend-functions #'gtags-mode-try-gtags))
   ...
   (t
    (remove-hook 'project-find-functions #'gtags-mode-try-gtags)
    (remove-hook 'xref-backend-functions #'gtags-mode-try-gtags)
